...
Armor Detector::FilterUniquenessArmor()
{
  Armor max_armor;
  
  max_armor.left_light.center.x = 0;
  max_armor.right_light.center.x = 0;

  for (const auto & armor : armors_) {
    if(armor.right_light.center.x - armor.left_light.center.x > max_armor.right_light.center.x - max_armor.left_light.center.x ){
      max_armor = armor;
    }
  }
